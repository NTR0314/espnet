...
base_paths2 = glob.glob("../exp/asr_33_raw_en_bpe5000_sp/*ms_asr_model_valid.acc.ave/")

results = {}
results_wer = {}
results_wer2 = {}
results_wer_masked_only = {}
results_wer_masked_only2 = {}
...
for base_path in base_paths2:
    # WER2
    mask_duration = re.search(r'(\d+)ms', base_path).group(1)
    wer_path = Path(base_path) / "test_clean" / "score_wer" / "result.txt"
    with open(wer_path, 'rb') as f:
        wer_lines_2 = f.readlines()
    wer = float([x for x in wer_lines_2 if 'Mean' in str(x)][0].split()[-3])
    results_wer2[int(mask_duration)] = wer
...
# Masked WER2
base_paths2 = glob.glob("../exp/asr_33_raw_en_bpe5000_sp/*ms_onlyMasked_asr_model_valid.acc.ave/")
for base_path in base_paths2:
    wer_path = Path(base_path) / "test_clean" / "score_wer" / "result.txt"
    mask_duration = re.search(r'(\d+)ms', base_path).group(1)
    with open(wer_path, 'rb') as f:
...
p2 = wer_ax.plot(positions, wers2, marker='D', label="WER of all tokens. 0.1CTC [0, 300] masking [-200, 200] random frames", color=color3)
...
p3 = wer_mo_ax.plot(positions, wers_masked_only2, marker='D', label="WER of masked tokens only. Half masked tokens are included. 0.1 CTC [0, 300] masking", color=color3)
...
fig.legend()
plt.savefig("koba_plot_libri_compare.png", bbox_inches='tight')
