...
then
    ...
    echo "arg1: basepath. Basepath is the folder where all the inference_folders are store, e.g., exp/asr_xx_.../"
...
if [[ -n "$4" ]]
then
    at="/project/OML/master_theses/ozink/Waseda/espnet/egs2/swbd/asr1/force_alignment/mfa_dev_aligned"
else
...
do
    ...
    then
        if [ -n "$3" ]
        then
            attn_path="${base_path}${ms_time}ms_${3}_asr_model_valid.acc.ave/org/train_dev/attn_dir"
        else
            attn_path="${base_path}${ms_time}ms_asr_model_valid.acc.ave/org/train_dev/attn_dir"
        fi
    else
        if [ -n "$3" ]
        then
            attn_path="${base_path}${ms_time}ms_${3}_asr_model_valid.acc.ave/eval2000/attn_dir"
        else
